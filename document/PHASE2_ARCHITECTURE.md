...
המפתחות נמצאים בתיקיית הפרויקט בקבצים הבאים:
- `google_vision_api.txt` - מפתח Google Cloud Vision
- `claude_api.txt` - מפתח Anthropic/Claude

#### טעינת המפתחות בקוד

המערכת תטען את המפתחות אוטומטית מהקבצים. ראה את הפונקציה `load_api_keys()` ב-`config/cloud_config.py`.

#### אלטרנטיבה: משתני סביבה

אם מעדיפים משתני סביבה במקום קבצים:
```bash
export GOOGLE_VISION_API_KEY="your-google-api-key"
...
המפתחות נמצאים בתיקיית הפרויקט:

| קובץ | תוכן |
|------|------|
| `google_vision_api.txt` | מפתח Google Cloud Vision API |
| `claude_api.txt` | מפתח Anthropic Claude API |

**חשוב:** 
- הקבצים האלה כבר קיימים בפרויקט
- הקוד ב-`cloud_config.py` טוען אותם אוטומטית
- **אל תעלה את הקבצים האלה ל-Git!** הוסף אותם ל-`.gitignore`
